...
class _ScheduleScreenState extends State<ScheduleScreen>
  ...
  final List<String> _categories = [
    'All', 'Transportation', 'Accommodation', 'Activity', 'Food', 'Meeting', 'Sightseeing', 'Shopping', 'Other'
  ];
  ...
  Future<void> _loadScheduleItems() async {
    ...
    try {
      final items = await _scheduleService.getScheduleItemsByTrip(widget.tripId);
      setState(() {
  ...
  void _filterItems() {
    ...
    if (_searchController.text.isNotEmpty) {
      ...
      filtered = filtered.where((item) =>
          ...
          item.location.toLowerCase().contains(query)).toList();
    }
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(widget.tripName, style: TextStyle(fontSize: 14, color: colorScheme.onPrimary.withValues(alpha: 0.8))),
          ],
        ),
        bottom: TabBar(
  ...
  Widget _buildTodayView() {
    ...
    final todayItems = _filteredItems.where((item) {
      return item.startTime.year == today.year && item.startTime.month == today.month && item.startTime.day == today.day;
    }).toList();
  ...
  Widget _buildUpcomingView() {
    ...
    final upcomingItems = _filteredItems.where((item) => item.startTime.isAfter(now) && !item.isCompleted).toList();
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('No schedule items found', style: TextStyle(fontSize: 18, color: colorScheme.onSurface.withValues(alpha: 0.6))),
          const SizedBox(height: 8),
          Text('Tap the + button to add your first schedule item', style: TextStyle(fontSize: 14, color: colorScheme.onSurface.withValues(alpha: 0.4))),
        ],
      ),
    );
  }
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: item.isCompleted ? Colors.green.withValues(alpha: 0.3) : priorityColor.withValues(alpha: 0.3), width: 2),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(categoryIcon, color: colorScheme.primary, size: 20),
                  ),
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            ...
                            color: item.isCompleted ? colorScheme.onSurface.withValues(alpha: 0.6) : colorScheme.onSurface,
                          ),
                        ),
                        if (item.location.isNotEmpty)
                          Row(
                            children: [
                              Icon(Icons.location_on, size: 14, color: colorScheme.onSurface.withValues(alpha: 0.6)),
                              const SizedBox(width: 4),
                              Expanded(child: Text(item.location, style: TextStyle(fontSize: 12, color: colorScheme.onSurface.withValues(alpha: 0.6)))),
                            ],
                          ),
                      ],
                    ),
                  ...
                  Container(
                    ...
                    child: Text(item.priority, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: priorityColor)),
                  ),
                ],
              ),
              const SizedBox(height: 12),
              if (item.description.isNotEmpty)
                Text(item.description, style: TextStyle(fontSize: 14, color: colorScheme.onSurface.withValues(alpha: 0.8)), maxLines: 2, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 12),
              Row(
                children: [
                  ...
                  const Spacer(),
                  IconButton(
                    icon: Icon(item.isCompleted ? Icons.check_circle : Icons.radio_button_unchecked, color: item.isCompleted ? Colors.green : colorScheme.outline),
                    onPressed: () => _toggleCompletion(item),
                  ),
                  IconButton(icon: Icon(Icons.edit, color: colorScheme.primary), onPressed: () => _editItem(item)),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (item.description.isNotEmpty) Text('Description: ${item.description}'),
            Text('Time: ${DateFormat('MMM dd, yyyy HH:mm').format(item.startTime)} - ${DateFormat('HH:mm').format(item.endTime)}'),
  ...
  void _showItemForm({ScheduleItem? item}) {
    showDialog(
      ...
      builder: (context) => ScheduleItemDialog(
        ...
        onSaved: () {
          _loadScheduleItems();
          _showSnackBar(item == null ? 'Schedule item added' : 'Schedule item updated', Colors.green);
        },
      ),
    );
  }
  ...
  Future<void> _toggleCompletion(ScheduleItem item) async {
    try {
      ...
      _loadScheduleItems();
      _showSnackBar(item.isCompleted ? 'Marked as pending' : 'Marked as completed', Colors.green);
    } catch (e) {
...
class ScheduleItemDialog extends StatefulWidget {
  ...
  const ScheduleItemDialog({super.key, required this.tripId, this.item, required this.onSaved});
...
class _ScheduleItemDialogState extends State<ScheduleItemDialog> {
  ...
  final List<String> _categories = ['Transportation', 'Accommodation', 'Activity', 'Food', 'Meeting', 'Sightseeing', 'Shopping', 'Other'];
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(widget.item == null ? 'Add Schedule Item' : 'Edit Schedule Item'),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              ListTile(
                title: Text('Start: ${DateFormat('MMM dd, yyyy HH:mm').format(_startTime)}'),
                trailing: const Icon(Icons.access_time),
                onTap: () => _selectDateTime(true),
              ),
              ListTile(
                title: Text('End: ${DateFormat('MMM dd, yyyy HH:mm').format(_endTime)}'),
                trailing: const Icon(Icons.access_time),
  ...
  Future<void> _selectDateTime(bool isStart) async {
    ...
    if (date != null) {
      if (!mounted) return;
      final time = await showTimePicker(context: context, initialTime: TimeOfDay.fromDateTime(isStart ? _startTime : _endTime));
      if (time != null) {
        setState(() {
